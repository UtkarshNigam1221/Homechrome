...
func (s *AnalyticsService) GetFunnelAnalytics(ctx context.Context, startDate, endDate string) (*domain.FunnelAnalytics, error) {
	...
	for i, step := range stepOrder {
		...
		if i > 0 && i-1 < len(stepOrder) && stepTotals[stepOrder[i-1]] > 0 {
...
func (s *AnalyticsService) GetEngagementAnalytics(ctx context.Context, startDate, endDate string) (*domain.EngagementAnalytics, error) {
	...
	totalSessions, totalBounces, durationSum, durationDays, pageViewCounts := aggregateEngagementRows(rows)
	...
	deviceBreakdown := s.getDeviceBreakdown(ctx, startDate, endDate)
	topPages := buildTopPages(pageViewCounts, 10)
...
// aggregateEngagementRows sums sessions, bounces, durations, and page views across daily rows.
func aggregateEngagementRows(rows []map[string]interface{}) (totalSessions, totalBounces int, durationSum float64, durationDays int, pageViewCounts map[string]int) {
...
// getDeviceBreakdown computes device type percentages from CUSTOMERS aggregates.
func (s *AnalyticsService) getDeviceBreakdown(ctx context.Context, startDate, endDate string) map[string]float64 {
	...
	if totalDevices > 0 {
		for device, count := range deviceCounts {
			breakdown[device] = float64(count) / float64(totalDevices) * 100
		}
	}
	return breakdown
}

// buildTopPages sorts page view counts and returns the top N pages.
func buildTopPages(pageViewCounts map[string]int, limit int) []domain.PageStats {
	...
	if len(topPages) > limit {
		topPages = topPages[:limit]
	}
	return topPages
}
