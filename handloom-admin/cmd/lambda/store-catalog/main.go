// Package main is the Lambda entry point for the Store Catalog service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-store-catalog")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeStoreCatalogDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewStoreCatalogRouter(r, deps.Handler)
