// Package main is the Lambda entry point for the Store Orders service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-store-orders")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeStoreOrdersDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewStoreOrdersRouter(r, deps.Handler, deps.CustomerAuthMiddleware)
