// Package main is the Lambda entry point for the Store Profile service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-store-profile")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeStoreProfileDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewStoreProfileRouter(r, deps.Handler, deps.CustomerAuthMiddleware)
