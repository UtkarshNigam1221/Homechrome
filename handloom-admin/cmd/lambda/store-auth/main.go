// Package main is the Lambda entry point for the Store Auth service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-store-auth")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeStoreAuthDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewStoreAuthRouter(r, deps.Handler, deps.CustomerAuthMiddleware)
