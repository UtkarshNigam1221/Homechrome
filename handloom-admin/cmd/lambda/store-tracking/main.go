// Package main is the Lambda entry point for the Store Tracking service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-store-tracking")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeStoreTrackingDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewStoreTrackingRouter(r, deps.Handler)
