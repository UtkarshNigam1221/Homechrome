// Package main is the Lambda entry point for the UTM link service
package main
...
func main() {
	bc := bootstrap.InitLambda("handloom-utm")
	defer bc.Shutdown()

	ctx := context.Background()

	deps, err := wire.InitializeUTMDeps(ctx, bc.Cfg)
	if err != nil {
	...
	router.NewUTMLinkRouter(r, deps.Handler)
