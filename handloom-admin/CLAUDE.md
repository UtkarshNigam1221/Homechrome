...
- **VAPID keys**: generate once per environment with `make vapid-keys`. Locally, `make ensure-vapid` (a prerequisite of `setup-local` and `run`) writes a keypair into `.env`, so push delivers from a fresh clone. Deployed, the public key and subject ride in as plain env vars while the **private key is read from SSM at runtime** — CDK gives the push Lambda `VAPID_PRIVATE_KEY_PARAM=/handloom/{env}/vapid-private-key` plus `ssm:GetParameter` on that path, and no other Lambda gets either, so the signing key never enters the CloudFormation template. Provision it out-of-band before deploying (`aws ssm put-parameter --type SecureString`). `VAPID_PRIVATE_KEY` still wins when set, for local and for a break-glass override. **Rotating the keypair invalidates every existing subscription** — browsers bind their subscription to the public key they subscribed with. An unreadable or empty key selects the dev gateway, which logs payloads instead of delivering them and now logs at ERROR when an SSM read fails.
