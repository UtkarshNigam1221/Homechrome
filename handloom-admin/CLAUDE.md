...
Handloom Admin is a Go serverless backend for the Homechrome handloom e-commerce platform. It powers both the admin dashboard and the B2C customer storefront. It runs as 25 Lambda services in production (12 admin + 9 store + 4 event workers) and a single monolithic server locally (port 8081). Go module: `github.com/handloom/admin`, Go 1.24.
...
#### PostgreSQL (Catalog)
Categories, products, inventory stored in PostgreSQL (RDS). Schema: `migrations/001_catalog_schema.sql` (auto-applied via Docker on first start). Repository implementations in `internal/repository/postgres/`.
